# Package index

## Visualization Blocks

Blocks for creating plots with ggplot2

- [`new_ggplot_block()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/new_ggplot_block.md)
  : Universal ggplot block with selectable visualization types
- [`new_facet_block()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/new_facet_block.md)
  : Facet Block
- [`new_theme_block()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/new_theme_block.md)
  : Theme customization block for ggplot2 plots
- [`new_grid_block()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/new_grid_block.md)
  : Grid Block

## Internal Functions

Internal UI components and helper functions

- [`new_ggplot_transform_block()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/new_ggplot_transform_block.md)
  : ggplot transform block constructor
- [`blockr.ggplot`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/blockr.ggplot-package.md)
  [`blockr.ggplot-package`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/blockr.ggplot-package.md)
  : blockr.ggplot: Interactive 'ggplot2' Visualization Blocks
- [`build_theme_choices()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/build_theme_choices.md)
  : Build theme choices list based on available packages
- [`get_theme_call()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/get_theme_call.md)
  : Get the base theme call for a given theme name
- [`palette_scale_call()`](https://bristolmyerssquibb.github.io/blockr.ggplot/reference/palette_scale_call.md)
  : Build a color-palette scale call for the theme block
